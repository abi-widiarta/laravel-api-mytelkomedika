@extends('layouts.layout_dashboard_client')
    ...
    <div class="h-full">
            ...
            <div class="h-96 flex flex-col w-[30%]">
                <div class="px-8 py-6 bg-white rounded-xl shadow-[0px_7px_50px_0px_rgba(198,203,232,0.2)]">
                    ...
                    <div class="flex items-start justify-between pb-4 mb-4 border-b-2 border-slate-100">
                        ...
                        <div class="text-end">
                            <p class="text-xs font-medium text-gray-500">{{ $data->tanggal_hari_ini}}</p>
                            <p class="text-xs font-medium text-gray-500">{{  \Carbon\Carbon::parse($data->jam_mulai_hari_ini)->format('H:i') }} - {{  \Carbon\Carbon::parse($data->jam_selesai_hari_ini)->format('H:i') }}</p>
                        </div>
                    </div>
                    <div class="grid w-full grid-cols-3 gap-4">
                        <div>
                            ...
                            <div class="flex flex-col items-start bg-[#FFE8E8] p-2 rounded-lg">
                                ...
                                <p class="font-bold text-[#FF5050] text-base">{{ $data->antrian_umum  == '-' ? '-' : 'No ' . $data->antrian_umum }}</p>
                            </div>
                        </div>
                        <div>
                            ...
                            <div class="flex flex-col items-start bg-[#E8F8FF] p-2 rounded-lg">
                                ...
                                <p class="font-bold text-[#41B6FF] text-base">{{ $data->antrian_mata == '-' ? '-' : 'No ' . $data->antrian_mata }}</p>
                            </div>
                        </div>
                        <div>
                            ...
                            <div class="flex flex-col items-start bg-[#DCFCE7] p-2 rounded-lg">
                                ...
                                <p class="font-bold text-[#3CD755] text-base">{{ $data->antrian_gigi  == '-' ? '-' : 'No ' . $data->antrian_gigi }}</p>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="mt-8">
            ...
            <div class="flex space-x-8">
                @foreach ($data->recommended_doctors as $doctor)
                    <a href="/lakukan-reservasi/detail?username={{ $doctor->username }}"
